...
Run all Uncontrolled climate scenarios and calculate objectives.
'''
...
def run_SWAT(file, rank):
    os.chdir(file)
    os.system('cp -r /scratch/smj5vup/omoScenarios/BestPolicyEachObjCC/Uncontrolled/SWATfiles ./') #copy swat files 
    os.system('cp Weather/* SWATfiles') # copy weather files 
    os.chdir("SWATfiles") # change to directory
    ...
    # copy files
    obj_name = file + "_UC_objs.txt"
    os.system("cp objs_file.txt /scratch/smj5vup/omoScenarios/BestPolicyEachObjCC/Objectives/Uncontrolled/%s" % obj_name)

    os.chdir('/sfs/lustre/bahamut/scratch/smj5vup/omoScenarios/BestPolicyEachObjCC/Uncontrolled/') # change back do original file 


comm = MPI.COMM_WORLD
rank = comm.Get_rank()
os.chdir('/sfs/lustre/bahamut/scratch/smj5vup/omoScenarios/BestPolicyEachObjCC/Uncontrolled/')
files = os.listdir()
